...
### 화면 한가운데 고정된 Box ###
	...
	void setup() {
		...
		float len = height / 2;
		origin = new Box(width / 2, height / 4, 40, 40, false);
		terminal = new Box(width / 2 + len * cos(radians(30)), height * 3 / 4, 40, 40, true);
		
		djd = new DistanceJoint();
		djd.bodyA = origin;
		djd.bodyB = terminal;
		djd.length = box2d.scalarPixelsToWorld(height / 2);
		djd.frequencyHz = 0;
		djd.dampingRatio = 1;
		dj = box2d.world.createJoint(djd)
	}
	...
	class Box {
		...
		Box(float x_, float y_, float w_, float h_, boolean isDynamic) {
			w = w_;
			h = h_;
