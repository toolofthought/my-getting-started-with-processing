...
#### 웹사이트 소스코드 불러오기 ####
	...
	//myWebSourceCode
	String URL = "http://m.blog.naver.com/yebin0219/40195095276";
...
실시간 급상승 검색어는 realrank라는 id로 구별이 가능하고 <li>태그 안에 정보가 위치합니다. id까지 지정해두었따면 분석은 식은 죽먹기입니다. id는 그 페이지 안에서 유일하게 결정되는 속성이니까요. 키워드는 id가 realrank인 <ol> 태그 아래 <li>태그 아래 <a> 태그에 저장되어 있습니다. <a> 태그의 [title] 속성을 따오면 쉽게 키워드명을 얻을 수 있겠습니다.
...
네. 프로세싱 혹은 자바는 대단합니다. 이제부터라도 열심히 사용합시다.
...
분석이 끝났으니 이제 selenium을 열심히 굴려볼 차례입니다.
...
void setup() {
	...
	inputDate.sendKeys(Keys.BACK_SPACE * 10);
...
selenium과 Jsoup이 함께하면 두려울 것이 없습니다. 날짜는 selenium으로 자동입력하고 웹서버가 입력된 날짜의 페이지를 출력하면 이를 Jsoup을 이용해 필요한 정보만 잘라내겠습니다.
...
이 자료는 이용자가 없는 한밤 중에 다운받았습니다. 따로 자료를 받지마시고 미리 받아둔 자료를 사용합시다.  
### ebb and flow 그림 그리기 ###
### silhouette ###
### wiggle ###